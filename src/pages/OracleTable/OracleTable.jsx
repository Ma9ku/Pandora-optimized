...
function OracleTable(props) {
    ...
    const getData = () => {
        ...
        console.log(req)
        axios.get('http://192.168.5.22:1415/get', {params: req}, {
    ...
    const countGet = () => {
        ...
        console.log(req)
        axios.get('http://192.168.5.22:1415/count', {params: req}, {
                cancelToken: request.token
            }).then(res => {
                setCount(res.data)
                setCounting(false)
    ...
    const download = () => {
        axios.get('http://192.168.5.22:1415/export-to-pdf', {responseType: 'blob'}).then(res=> {
...
function ResultTable(props) {
    ...
    const downloadSchema = (row) => {
        ...
        axios.get('http://192.168.5.22:8081/export-to-pdf/' + row.messOfmId + '/' + row.memberId, {responseType: 'blob'}).then(res=> {
            const url = window.URL.createObjectURL(new Blob([res.data]))
            const link = document.createElement('a')
            link.href = url
            link.setAttribute('download', 'document.pdf')
